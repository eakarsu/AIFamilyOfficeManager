...
// an HMAC-SHA256 signature in the X-FamilyOffice-Signature header, and records the
...
async function fireWebhook(event, payload) {
  ...
  for (const sub of matching) {
    ...
    const headers = {
      'X-FamilyOffice-Event': event,
      'X-FamilyOffice-Signature': `sha256=${sig}`,
      'X-FamilyOffice-Webhook-Id': String(sub.id),
    };
